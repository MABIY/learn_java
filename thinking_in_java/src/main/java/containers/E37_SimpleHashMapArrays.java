...
/**
 * @author lh
 * {Args: 100 50000} small to keep build testing short
 *
 * Modify SimleHashMap to use ArrayLists instead
 * of LinkedLists.Modify MapPerformance.java to
 * compare the performance of the two implementions
 **/
...
public class E37_SimpleHashMapArrays {
    public static void main(String[] args) {
        args = new String[]{"100","50000"};
        if (args.length > 0) {
            Tester.defaultParams = TestParam.array(args);
        }
        Tester.run(new SimpleHashMap4<>(), tests);
        Tester.run(new SimpleHashMap5<>(),tests);
        Tester.run(new SimpleHashMap6<>(),tests);
    }
}
